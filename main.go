...
const (
	...
	defaultNamespace = "openshift-ptp"
	defaultPod       = "linuxptp-daemon-*"
	defaultContainer = "linuxptp-daemon-container"
)
...
func getLogsFileNamesForNode(nodeName string) []string {
	...
	_, err = expecter.Send(fmt.Sprintf("find /host/var/log/pods/%s_%s/%s/*log*\n", nsGlob, podGlob, containerName))
	check(err)
...
func buildSingleLog(fromFiles []string, since time.Time) {
	...
	filesToClean = append(filesToClean, reverseFilename)
...
func cleanup() {
	...
	for _, fileName := range filesToClean {
		log.Debugf("Cleanup %s", fileName)
		_ = os.Remove(fileName)
	}
}

func main() {
	flag.StringVar(&kubeconfigPath, "kubeconfig", "", "Path to kubeconfig. Required.")

	flag.StringVar(&outputPath, "output", "", "Output file for final logs. Target must exist. Required")

	flag.StringVar(&nsGlob, "namespace", defaultNamespace, "Glob that matches the Namespace from which to get logs")

	flag.StringVar(&podGlob, "pod", defaultPod, "Glob that matches the Pod from which to get logs")

	flag.StringVar(&containerName, "container", defaultContainer, "Container from which to get logs")

	flag.DurationVar(
		&logWindow,
		"duration",
		defaultLogWindow,
		fmt.Sprintf("Optional. Duration of logs to finally output. Default %s.", defaultLogWindow),
	)
	...
	flag.StringVar(
		...
		"temp",
		".",
		"Optional. Specify the location to output uncombined logs. Must exist. Default to working directory.",
	)

	flag.Parse()

	if kubeconfigPath == "" {
		log.Error("Kubeconfig path is Required")
		flag.Usage()
		os.Exit(1)
	}
	if outputPath == "" {
